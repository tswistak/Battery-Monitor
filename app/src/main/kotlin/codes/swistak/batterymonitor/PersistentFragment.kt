...
class PersistentFragment : Fragment() {
    ...
    override fun onStart() {
        ...
        if (!spMain.getBoolean(
                SettingsFragment.KEY_MIGRATED_SERVICE_DESIRED, false
            )
        ) spMain.edit {
            putBoolean(SettingsFragment.KEY_MIGRATED_SERVICE_DESIRED, true)
        }
    }
    ...
    override fun onResume() {
        ...
        if (Build.VERSION.SDK_INT >= 33 && ContextCompat.checkSelfPermission(
            ...
            if (spMain.getBoolean(SettingsFragment.KEY_FIRST_RUN, true)) {
        ...
        if (spMain.getBoolean(SettingsFragment.KEY_FIRST_RUN, true)) {
            spMain.edit { putBoolean(SettingsFragment.KEY_FIRST_RUN, false) }
        }
            ...
            if (BatteryInfoService.supportsLiveUpdates()) {
                ...
                builder.setTitle(R.string.app_full_name)
                    .setMessage(R.string.notifications_onboarding_message).setPositiveButton(
                        ...
                        if (pf != null) {
                            pf.spMain.edit {
                                putBoolean(SettingsFragment.KEY_FIRST_RUN, false)
                            }
    ...
    fun loadSettingsFiles() {
        ...
        spService = requireActivity().getSharedPreferences(
            SettingsFragment.SP_SERVICE_FILE, Context.MODE_PRIVATE
        )
        spMain = requireActivity().getSharedPreferences(
            SettingsFragment.SP_MAIN_FILE, Context.MODE_PRIVATE
        )
    }
