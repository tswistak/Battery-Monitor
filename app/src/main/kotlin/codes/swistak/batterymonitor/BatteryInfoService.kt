...
class BatteryInfoService : Service() {
    ...
    private val runChipSwitch: Runnable = object : Runnable {
        override fun run() {
            ...
            interval = try {
                settings.getString(
                    SettingsFragment.KEY_CHIP_SWITCHING_INTERVAL, "5"
                )!!.toInt()
    ...
    private fun loadSettingsFiles() {
        settings = getSharedPreferences(SettingsFragment.SETTINGS_FILE, MODE_PRIVATE)
    ...
    private fun update(intent: Intent?) {
        ...
        predictor!!.setPredictionType(
            settings.getString(
                SettingsFragment.KEY_PREDICTION_TYPE, Str.defaultPredictionType
            )!!
        )
    ...
    private fun roundedTemperatureValue(): Int {
        val convertF = settings.getBoolean(
            SettingsFragment.KEY_CONVERT_F, res.getBoolean(R.bool.default_convert_to_fahrenheit)
        )
    ...
    private fun chipContentText(): String {
        var content: String = settings.getString(
            SettingsFragment.KEY_CHIP_CONTENT, res.getString(R.string.default_chip_content)
        )!!
    ...
    private fun liveUpdateMode(): String {
        val mode: String = settings.getString(
            SettingsFragment.KEY_LIVE_UPDATE_DISPLAY,
    ...
    private fun shouldKeepMainNotificationWithLiveUpdate(): Boolean {
        return settings.getBoolean(
            SettingsFragment.KEY_LIVE_UPDATE_KEEP_MAIN_NOTIFICATION, false
        )
    }
    ...
    private fun shouldRunChipSwitcher(): Boolean {
        ...
        return "switching" == settings.getString(SettingsFragment.KEY_CHIP_CONTENT, "")
    }

    private fun shouldShowChipChargingIndicator(): Boolean {
        if (!settings.getBoolean(
                SettingsFragment.KEY_CHIP_INDICATE_CHARGING, true
            )
        ) return false
    ...
    private fun lineFor(key: String): String? {
        val req: String = settings.getString(
            key, if (key == SettingsFragment.KEY_TOP_LINE) "remaining" else "vitals"
        )!!
        ...
        if (info!!.prediction.whatHappened == BatteryInfo.Prediction.NONE) {
            ...
            if (predicted.days > 0) line = Str.nDaysMHours(predicted.days, predicted.hours)
                ...
                val verbosity: String = settings.getString(
                    SettingsFragment.KEY_TIME_REMAINING_VERBOSITY,
    ...
    private fun iconForLegacyMainNotification(): Int {
        val content = contentPreference(
            SettingsFragment.KEY_ICON_CONTENT, res.getString(R.string.default_icon_content)
        )
        val indicateCharging = settings.getBoolean(SettingsFragment.KEY_INDICATE_CHARGING, true)
        val showUnit = settings.getBoolean(SettingsFragment.KEY_SHOW_ICON_UNIT, true)
        ...
        if (settings.getBoolean(SettingsFragment.KEY_ENABLE_LOGGING, true)) {
            ...
            if (info!!.status != info!!.lastStatus && info!!.lastStatus == BatteryInfo.STATUS_UNPLUGGED) {
                val maxLogAge = settings.getString(
                    SettingsFragment.KEY_MAX_LOG_AGE, Str.defaultMaxLogAge
                )!!.toInt()
    ...
    private fun handleUpdateWithSameStatus() {
        if (settings.getBoolean(
                SettingsFragment.KEY_ENABLE_LOGGING, true
            )
