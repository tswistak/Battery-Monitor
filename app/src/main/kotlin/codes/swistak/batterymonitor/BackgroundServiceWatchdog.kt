    ...
    companion object {
        ...
        fun recordHeartbeat(context: Context) {
            context.getSharedPreferences(
                SettingsFragment.SP_SERVICE_FILE,
        ...
        fun isServiceDesired(context: Context): Boolean {
            val settings = context.getSharedPreferences(
                SettingsFragment.SETTINGS_FILE,
                Context.MODE_PRIVATE
            )
            if ("always" == settings.getString(
                    SettingsFragment.KEY_AUTOSTART,
                    "auto"
                )
            ) return true

            val mainPreferences = context.getSharedPreferences(
                SettingsFragment.SP_MAIN_FILE,
                Context.MODE_PRIVATE
            )
            if (mainPreferences.getBoolean(
                    SettingsFragment.KEY_MIGRATED_SERVICE_DESIRED,
                    false
                )
            ) {
